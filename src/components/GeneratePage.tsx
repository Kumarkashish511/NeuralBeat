...
import { Swords, Plus, ChevronLeft, ChevronRight, Play, Pause, SkipBack, SkipForward, Heart, Share2, Download, Volume2, Home, Disc, MessageSquarePlus } from "lucide-react";
...
const GeneratePage = () => {
  ...
  useEffect(() => {
    const handleScroll = () => {
      // Hide header when scrolled down more than 50px
  ...
  return (
    <section className="min-h-screen bg-gradient-hero relative overflow-hidden pt-14">
      ...
      <header className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${!isVisible ? 'pointer-events-none' : ''}`}>
        <div className={`container mx-auto px-6 h-20 flex items-center justify-between relative transition-opacity duration-300 ${isVisible ? 'opacity-100' : 'opacity-0'}`}>
          <div className="flex items-center gap-8">
            <div className="flex items-center gap-3 cursor-pointer group" onClick={() => window.scrollTo({ top: 0, behavior: 'smooth' })}>
              ...
              <span className="font-display text-2xl font-bold text-foreground tracking-tight group-hover:tracking-wide transition-all duration-300">
                Aether
              </span>
            </div>
            ...
            <nav className="hidden md:flex items-center gap-2 absolute left-1/2 -translate-x-1/2 backdrop-blur-md border border-foreground/10 rounded-full p-1.5 ">
              ...
              >
                <Home className="w-4 h-4 text-foreground/80 group-hover:text-primary transition-colors" />
              ...
              <button
                ...
                className="group flex items-center gap-2 px-3 py-2 rounded-full hover:bg-foreground/10 transition-all duration-300"
              >
                <Disc className="w-4 h-4 text-foreground/80 group-hover:text-primary transition-colors" />
                <span className="max-w-0 overflow-hidden group-hover:max-w-xs text-sm font-medium text-foreground/90 whitespace-nowrap transition-all duration-300 ease-in-out">
                  AetherVault
                </span>
              </button>
              ...
              <button
                className="group flex items-center gap-2 px-3 py-2 rounded-full hover:bg-foreground/10 transition-all duration-300"
              >
                <MessageSquarePlus className="w-4 h-4 text-foreground/80 group-hover:text-primary transition-colors" />
                <span className="max-w-0 overflow-hidden group-hover:max-w-xs text-sm font-medium text-foreground/90 whitespace-nowrap transition-all duration-300 ease-in-out">
                  Add Reviews
                </span>
              </button>
        ...
        <div className="text-center mx-auto">
          {isGenerating ? (
            <>
              ...
              {/* Progress Section */}
              <div className="bg-white/10 backdrop-blur-xl border border-white/20 rounded-3xl p-10 max-w-xl mx-auto mb-12 flex flex-col items-center text-center">
                ...
                {/* Status Text */}
                <h3 className="text-white text-2xl font-bold mb-3 animate-pulse">
                  {(() => {
                    switch (generationStatus) {
                      case 'GENERATING_LYRICS': return 'Creating Lyrics...';
                      case 'LYRICS_COMPLETE': return 'Lyrics Complete';
                      case 'PREPARING_AUDIO': return 'Preparing Audio...';
                      case 'PROMPT': return 'Processing Prompt...';
                      case 'TASK_SENT': return 'Sending to Engine...';
                      case 'GENERATING': return 'Generating Audio...';
                      case 'DECOMPRESSING': return 'Finalizing...';
                      case 'SAVING': return 'Saving Files...';
                      case 'SUCCESS': return 'Complete!';
                      case 'FAILURE': return 'Generation Failed';
                      default: return generationStatus;
                    }
                  })()}
                </h3>

                <p className="text-white/70 text-lg mb-6 max-w-xs mx-auto leading-relaxed">
                  {backendInfo || 'We are crafting your unique song based on your prompt.'}
                </p>

                <div className="flex items-center gap-2 text-white/50 text-sm bg-white/5 px-4 py-2 rounded-full">
                  <div className="w-2 h-2 rounded-full bg-blue-400 animate-ping" />
                  Estimated time: {progressPercentage < 50 ? '20s' : '10s'}
                </div>
              </div>
            </>
          ) : (
